...
exports.sendRejectAdmission = async (user, team, callback) => {
  ...
  const mailData = {
    ...
    to: email,
    subject: `Aufnahme ins Team ${teamName}`,
    text: `Lieber ${firstName}, \n\ndeine Anfrage zur Aufnahme ins Team ${teamName} wurde durch den Team Captain leider abgelehnt.`,
    html: `<p><b>Lieber ${firstName}</b>,</br></br>deine Anfrage zur Aufnahme ins Team ${teamName} wurde durch den Team Captain leider abgelehnt.`
  };
...
exports.sendRemovedFromTeam = async (user, team, callback) => {
  const email = user.email;
  const firstName = user.first_name;
  const teamName = team.team_name;
 
  const askForAdmissionLink = `${variables.base_url}:${variables.port}/api/v1/askForAdmission/${team.id}/${user.id}`;


  const mailData = {
    from: "teams@bigpoints.de",
    to: email,
    subject: `Austritt aus Team ${teamName}`,
    text: `Lieber ${firstName}, \n\ndu wurdest aus dem Team ${teamName} entfernt. Sollte das ein Fehler sein, kontaktiere bitte deinen Teamcaptain..`,
    html: `<p><b>Lieber ${firstName}</b>,</br></br>du wurdest aus dem Team  ${teamName} entfernt. Sollte das ein Fehler sein, kontaktiere bitte deinen Teamcaptain.`,
  };
